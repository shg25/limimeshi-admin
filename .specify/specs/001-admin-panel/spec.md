...
1. **Given** 管理者がログイン済み, **When** 新しいチェーン店（ID、名前、ふりがな、公式URL、ロゴURL）を入力して保存, **Then** チェーン店が一覧に表示され、システムに永続化される
...
4. **Given** チェーン店登録フォーム, **When** 管理者がID、チェーン名、またはふりがなを未入力で保存, **Then** エラーメッセージが表示され保存できない
5. **Given** チェーン店登録フォーム, **When** 管理者がIDに英数字小文字・ハイフン以外の文字を入力, **Then** エラーメッセージが表示され保存できない
...
5. **Given** キャンペーン登録フォーム, **When** 管理者がキャンペーン名、所属チェーン、または販売開始日時を未入力で保存, **Then** エラーメッセージが表示され保存できない
6. **Given** キャンペーン登録フォーム, **When** 管理者が販売終了日時を未入力で保存, **Then** キャンペーンは販売終了日時なしで保存され、ステータスが「予定」または「開始から◯日経過」と自動判定される
7. **Given** 販売終了日時が未入力で販売開始日時から1ヶ月以上経過, **When** 管理者がキャンペーン一覧を表示, **Then** ステータスに経過期間（「開始から1ヶ月経過」「開始から2ヶ月経過」など）が表示される
...
- **チェーン店削除**: Phase2では削除機能を提供しない。16店舗のみで削除頻度が極めて低いため、誤削除のリスクを避ける。必要な場合はFirebase Consoleから手動削除。削除ボタンはクリック時に警告メッセージを表示する。
- **一括操作の無効化**: チェーン店・キャンペーンの一括削除・一括編集はPhase2では無効化。誤操作による大量データ消失を防止。
- **チェーン店IDの変更不可**: 登録後のID変更はURLや参照整合性に影響するため、編集画面ではIDを読み取り専用で表示。変更が必要な場合はFirebase Consoleで対応。
...
- **FR-004**: システムはチェーン店の新規登録・編集機能を提供しなければならない（ID、チェーン名、ふりがな、公式URL、ロゴURL）
...
- **FR-007**: システムはチェーン店IDを英数字小文字とハイフンのみに制限しなければならない（URL用途）
- **FR-008**: システムはチェーン店IDを登録後は変更不可（読み取り専用）として扱わなければならない
...
- **Chain（チェーン店）**: 大手チェーン店の情報を表す。ID（URL用、英数字小文字+ハイフン、変更不可）、チェーン名、ふりがな、公式サイトURL、ロゴ画像URLを持つ。キャンペーンの親エンティティ。
...
- **検索・フィルタ機能**: チェーン名やキャンペーン名での検索・フィルタリング機能。Phase2は16店舗・32キャンペーン程度のため、スクロールで十分対応可能。
